DROP TABLE IF EXISTS bilayer.fpd_monthly_net_movements_scheduled_query;

CREATE TABLE bilayer.fpd_monthly_net_movements_scheduled_query AS
...
, SUM(pageview_number_of_devices) pageview_number_of_devices
 FROM bilayer.fpd_prospect_visitors_scheduled_query 
...
FROM bilayer.fpd_raw_monthly_net_movements_scheduled_query m
LEFT JOIN prospect_visitors v 
  ON CAST(m.month AS VARCHAR) = CAST(v.visit_time_stamp_utc_month AS VARCHAR)
WHERE 
...
, 0 conversion_rate
FROM bilayer.fpd_raw_monthly_net_movements_scheduled_query
...
WHERE TO_DATE(month, 'YYYY-MM') < DATE_TRUNC('month', CURRENT_DATE);
