...
WITH c AS (
  WITH b AS (
    WITH a AS (
      WITH revenue AS (
        WITH adbook AS (
          SELECT
            ...
            line_item_end_date
          FROM
            `ft-advertising.product_analytics.booking_system_product`
          WHERE
            1 = 1
        ...
        ad_imps AS (
          SELECT
            COALESCE(
              IF(LOWER(user_guid) IN ("", "null", "unknown"), NULL, user_guid), 
              IF(LOWER(spoor_id) IN ("", "null", "unknown"), NULL, spoor_id)
            ) AS ft_user_id,
            DATE(time_day) AS date,
            line_item_id
          FROM `ft-advertising.master_ads_events.master_ads_events` 
        ...
        gl AS (
          ...
          FROM `ft-advertising.revenue.ads_user_revenue_base`
      ...
      known_users AS (
        ...
        FROM
          `ft-bi-team.BI_Layer_Integration.known_user_daily_status`
        WHERE
  ...
  SELECT
    ...
    date_year,
    date_month,
    DATE(date_year, date_month, 1) AS date,
    median_user_count,
  ...
  WITH b AS (
    WITH a AS (
      WITH revenue AS (
        ...
        FROM
            `ft-advertising.ads_reference.user_guid_revenue` --new table: `ft-advertising.ads_reference.Ads_user_daily_revenue`
        WHERE
          date >= "2022-01-01"
        GROUP BY
          1,2
